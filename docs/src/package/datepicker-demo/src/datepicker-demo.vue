<template>
  <div
    class="better-datepicker-wrapper"
    style="
  ...
  >
    <div
      class="better-datepicker-header"
      :style="{ color: header.value }"
      style="width: 298px; text-align: center"
    >
      <svg
        viewBox="0 0 1024 1024"
        style="position: absolute; left: 30px; width: 14px; height: 14px"
      >
        <path
      ...
      <span style="display: none">2020 - 2029</span
      ><span
        class="better-datepicker-pointerCursor"
    ...
    <table
      class="better-datepicker-date better-datepicker-week"
      style="
        text-align: center;
        padding: 5px 20px;
        width: 320px;
    ...
    >
      <thead>
        <th
          :style="{ color: th.value }"
          v-for="name in thName"
          style="height: 40px; width: 40px; padding: 3px 0"
        >
          {{ name }}
        </th>
      </thead>
      <tbody>
        <tr v-for="row in 6">
          <td
            ...
            style="height: 40px; width: 40px; padding: 3px 0"
            :style="{
              color: getDay(row, col) === 3 ? theme.value : tbody.value,
            }"
            :class="{
              'better-datepicker-pre': getDay(row, col) < 5,
              'better-datepicker-next': getDay(row, col) > 34,
              next: getDay(row, col) > 32,
              'better-datepicker-range-start': row === 3 && col === 1,
              'better-datepicker-range-end': row === 3 && col === 7,
              'better-datepicker-inRange': row === 3 && col > 1 && col < 7,
            }"
          >
            <div
              :style="{
