...
describe('sheetRuleStatus', () => {
  ...
  it('should get keyframes rules', () => {
    ...
    expect(hiddenRules).toEqual(
      `@keyframes hidden { 0% {opacity: 1;transform: translate(0,0) scaleY(1);} 100% {opacity: 0;visibility: hidden;transform: translate(0,0) scaleY(.8);} }`
    )

    const showRules = sheetRule[1](transform)
    expect(showRules).toEqual(
      `@keyframes show { 0% {display: block;opacity: 0;transform:scaleY(.8) translate(0,0);} 100% {display: block;opacity: 1;transform: scaleY(1) translate(0,0);} }`
    )
  })
})
