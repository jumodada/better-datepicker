...
describe('Date', () => {
  ...
  it('should get the actions of the previous month', () => {
    expect(getPre(1, 2020)).toEqual([12, 2019])
    expect(getPre(2, 2020)).toEqual([1, 2020])
  })

  it('should get the actions of the next month', () => {
    expect(getNext(12, 2020)).toEqual([1, 2021])
    expect(getNext(2, 2020)).toEqual([3, 2020])
  })

  describe('isInRange', () => {
    it('should return whether actions is in range', () => {
      expect(isInRange('2020/10/22', '2020/9/22', '2020/10/3')).toEqual(
        'in-range'
      )
  ...
  it('should get the number of days in the month', () => {
    expect(daysInMonth(2020, 9)).toBe(30)
  })
  ...
  it('should get the week number of the current actions', () => {
    locale(zhCN as any)
    const { state } = createDatePicker(document.createElement('input'))
    expect(
      getYearWeek(new Date('2019-06-10T00:00:00.000Z'), state.locale)
    ).toBe(24)
    expect(
      getYearWeek(new Date('2019-01-01T00:00:00.000Z'), state.locale)
    ).toBe(1)
    expect(
      getYearWeek(new Date('2019-01-06T00:00:00.000Z'), state.locale)
    ).toBe(1)
    expect(
      getYearWeek(new Date('2020-01-06T00:00:00.000Z'), state.locale)
    ).toBe(2)

    expect(
      getYearWeek(new Date('2020-12-26T00:00:00.000Z'), state.locale)
    ).toBe(52)
    expect(
      getYearWeek(new Date('2020-12-28T00:00:00.000Z'), state.locale)
    ).toBe(53)

    expect(
      getYearWeek(new Date('2023-01-01T00:00:00.000Z'), state.locale)
    ).toBe(52)
  })
})
